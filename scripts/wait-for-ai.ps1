...
Write-Host "[*] Waiting for AI service at $AiUrl/health (max ${MaxWaitSeconds}s)..."
...
while ((Get-Date) -lt $deadline) {
    try {
        ...
        Write-Host "[.] AI status: $($health.status) — $($health.message)"
    } catch {
        Write-Host "[.] AI not ready yet: $($_.Exception.Message)"
    }
